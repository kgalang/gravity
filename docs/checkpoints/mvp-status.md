...
| CP4 | in_progress | `pi-*` dependencies are installed and `/wiggs` is wired to a `pi-coding-agent` Claude loop with per-turn `store/` skill loading, dbt context loading, and built-in read/bash truncation behavior. Runtime now includes ingress-binding-driven non-slash triggers (`app_mention`, `message` thread/DM), split run dimensions (`trigger_kind`, `surface`, `entrypoint`), `gravity.sessions` metadata writes, proactive trigger scheduling (`cron` + `heartbeat`), and source-event idempotency guard checks. This intentionally includes a pulled-forward CP10 foundation slice; checkpoint numbering stays unchanged. Remaining CP4 work is live Slack E2E validation and tuning against target prompts before CP4 close. | `docs/plans/active/2026-02-18-cp4-wiggs-e2e.md` |
...
- After CP4 closes, run deferred CP10 validation/hardening (`channel_thread`/`dm` delivery validation, `replay/backfill`, manual wake, quiet hours) and then mark CP10 complete.
