...
| CP4 | in_progress | `pi-*` dependencies are installed and `/wiggs` is wired to a `pi-coding-agent` Claude loop with per-turn `store/` skill loading, dbt context loading, and built-in read/bash truncation behavior. Remaining CP4 work is live Slack E2E validation against target prompts. | `docs/plans/active/2026-02-18-cp4-wiggs-e2e.md` |
