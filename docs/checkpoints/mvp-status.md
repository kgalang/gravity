...
| CP5.1 | in_progress | Rearchitecture parity checkpoint is active. Step 1 is complete (`defineConfig`/`defineAgent` contracts + typed `agents` registry with collision guards); remaining work is compiled declarations + runtime cutover + legacy module removal + parity verification before resuming CP6. | `docs/plans/active/2026-02-18-cp5-1-rearchitecture-parity.md`, `docs/architecture/rearchitecture-decision.md`, `agents/index.ts`, `agents/contracts.ts` |
