...
| TD-006 | No `npm run doctor` command for runtime/live-definition misconfiguration checks | Misconfigured agent specs, connector paths, ingress bindings, required CLIs, or runtime writability can fail late at runtime | Add `npm run doctor` with scoped runtime checks that explicitly avoid overlap with TypeScript/static guarantees |
